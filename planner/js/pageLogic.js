...
function init() {
    ...
    createTable("school-mat-table", ["BD_1", "BD_2", "BD_3", "BD_4", "TN_1", "TN_2", "TN_3", "TN_4"], 0,
        ["Hyakkiyako", "Red Winter", "Trinity", "Gehenna", "Abydos", "Millennium", "Arius", "Shanhaijing", "Valkyrie", "Highlander", "Wildhunt", "Odyssey"], 0,
...
function populateCharResources(charId) {
    ...
    let miscMatsWrapper = document.getElementById('char-miscMats-wrapper'); // For favor/Gifts and Potential/Limit Break
    ...
    while (miscMatsWrapper.children.length > 0) {
        miscMatsWrapper.children[0]._tippy.destroy();
        miscMatsWrapper.children[0].remove();
    }
    ...
    if (resources) {
        ...
        for (key in resources) {
            ...
            if (matName && matName != "Secret") {
                ...
                else if (matName.includes("favor") || matName.includes("Workbook")) {
                    miscMatsWrapper.appendChild(wrapDiv)
                }
                else {
...
function GenerateModelVariablesFast(multiplier) {
    let rates = misc_data.gear_rates;
...
function SortStudents(students, sortType) {
    let academyOrder = {
        "Odyssey": 14, "Wildhunt": 13, "Highlander": 12, "Hyakkiyako": 11, "RedWinter": 10, "Trinity": 9, "Gehenna": 8, "Abydos": 7,
